package com.xiyan.mianshihou.service.impl;
...
import com.xiyan.mianshihou.mapper.QuestionBankQuestionMapper;
import com.xiyan.mianshihou.model.dto.questionBankQuestion.QuestionBankQuestionQueryRequest;
import com.xiyan.mianshihou.model.entity.Question;
import com.xiyan.mianshihou.model.entity.QuestionBank;
import com.xiyan.mianshihou.model.entity.QuestionBankQuestion;
import com.xiyan.mianshihou.model.entity.User;
import com.xiyan.mianshihou.model.vo.QuestionBankQuestionVO;
import com.xiyan.mianshihou.model.vo.UserVO;
import com.xiyan.mianshihou.service.QuestionBankQuestionService;
import com.xiyan.mianshihou.service.QuestionBankService;
import com.xiyan.mianshihou.service.QuestionService;
import com.xiyan.mianshihou.service.UserService;
import com.xiyan.mianshihou.utils.SqlUtils;
...
public class QuestionBankQuestionServiceImpl extends ServiceImpl<QuestionBankQuestionMapper, QuestionBankQuestion> implements QuestionBankQuestionService {
    ...
    @Resource
    @Lazy  // 防止循环依赖
    // 因为 QuestionService 和 QuestionBankQuestionService 互相引用，会导致循环依赖问题，让项目无法启动。
    // 可以通过给 QuestionBankQuestionServiceImpl 中引用 QuestionService 的位置加上 @Lazy 注解解决。
...
//        }
    ...
    public void batchAddQuestionsToBank(List<Long> questionIdList, long questionBankId, User loginUser) {
        ...
        // 自定义线程池（IO 密集型线程池），对该方法的线程池，如果要多个方法都用该线程池就要将它定义成Bean，参考智能BI项目的线程池
        ThreadPoolExecutor customExecutor = new ThreadPoolExecutor(
                ...
                new ThreadPoolExecutor.CallerRunsPolicy() // 拒绝策略：由调用线程处理任务，即当线程池无法处理新提交的任务时，会将任务重新交给调用线程执行。但是，这种策略可能会导致调用线程阻塞，因此需要注意调整线程池的大小以避免这种情况。
        );
        ...
        for (int i = 0; i < totalQuestionListSize; i += batchSize) {
            ...
            CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                ...
                return null;
            })*/;  // 比如某个任务出现异常时，其他任务可能正在执行，产生不确定的影响。对此，可以根据情况给异步任务补充异常处理行为，通过 exceptionally 方法就能实现
            futures.add(future);
        }
        // 等待所有批次完成操作
